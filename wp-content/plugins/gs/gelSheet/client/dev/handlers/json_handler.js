...
*/		self.importFontStyles(data.fontStyles);
	...
	self.importSheet = function(configs,data){ //TODO:configs must be setted on jsondata
		...
		for(var i=0;i<cells.length;i++){
			sheet.setFormula(cells[i].dataRow,cells[i].dataColumn,stripslashes(cells[i].cellFormula), true);
			sheet.setCellFontStyleId(cells[i].dataRow,cells[i].dataColumn,cells[i].fontStyleId, true);
		}
		return sheet;
	};
