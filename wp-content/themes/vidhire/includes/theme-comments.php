...
function jr_job_comment_template($comment, $args, $depth) {
    ...
    <?php else : ?>
        ...
        <?php comment_text(); ?>
                        <text>
                        <span class="comment-author-link"><text>By:</text> <strong><?php comment_author_link(); ?></strong></span> 
                        &nbsp;&nbsp;&nbsp;&nbsp;<text>Regarding:</text>
                        <a href="<?php echo htmlspecialchars(get_comment_link($comment->comment_ID)) ?>"><?php the_title() ?></a>
                        </text>
                    </p>
