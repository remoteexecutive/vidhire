...
function jr_top_nav_links( $items = '', $menu = null) {
	...
	if (is_user_logged_in()) {
		...
		if ( JR_Dashboard_Page::get_id() && is_user_logged_in()) :
			...
			$items .= '<li class="right"><a href="'.get_permalink( JR_User_Profile_Page::get_id() ).'">'.__('Profile', APP_TD).'</a></li>';
			$items .= '<li class="right ';
			if ( is_page( JR_Dashboard_Page::get_id() ) ) $items .= 'current_page_item';
			$items .= '"><a href="'.get_permalink( JR_Dashboard_Page::get_id() ).'">'.__('Dashboard', APP_TD).'</a></li>';
    
     if (is_user_logged_in() && current_user_can('can_submit_resume')) {
     	$items .= '<li class="right ';
			if (  is_post_type_archive('job_listing') ) $items .= 'current_page_item';	
