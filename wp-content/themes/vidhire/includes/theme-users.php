...
Star Resumes
*/
function jr_star_resumes() {
global $post;

if(isset($_GET['star-resume']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 

  $_starred_resumes = get_user_meta(get_current_user_id(), '_starred_resumes',true);
	if(!is_array($_starred_resumes)) $_starred_resumes = array();
  
  if ($_GET['star-resume']=='true') :
  	if(!in_array($post->ID,$_starred_resumes)) : $_starred_resumes[] = $post->ID; endif;
  
  elseif($_GET['star-resume']=='second-highest') :
  	if(!in_array($post->ID,$_starred_resumes)) : $_starred_resumes[] = $post->ID; endif;
  	if(!in_array('second-highest',$_starred_resumes)) : $_starred_resumes[] = 'second-highest'; endif;
  
  else :
  	//$_starred_resumes = array_diff($_starred_resumes, array($post->ID));	
  	$_starred_resumes = [];
  endif;  
  
  
  
  update_user_meta(get_current_user_id(),'_starred_resumes',$_starred_resumes);
endif;  
}

add_action('appthemes_before_post', 'jr_star_resumes');

/*
Processing Statuses
*/
function jr_processing_status() {
global $post;

if(isset($_GET['skills-required']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 
	
  $_skills_required = get_user_meta(get_current_user_id(), '_skills_required',true);
	if(!is_array($_skills_required)) $_skills_required = array();
  
  if ($_GET['skills-required']=='true') :
  	if(!in_array($post->ID,$_skills_required)) : 
  			$_skills_required[] = $post->ID; 
  	
  	endif;
  
  elseif ($_GET['skills-required']=='insufficient') :
  	if(!in_array('insufficient',$_skills_required)) : 
  			$_skills_required[] = 'insufficient'; 
  	
  	endif;
  
  else :
  
  	//$_skills_required = array_diff($_skills_required, array($post->ID));
  	$_skills_required = [];
  endif;  
  
  update_user_meta(get_current_user_id(),'_skills_required',$_skills_required);
  
endif;  

if(isset($_GET['fast-track']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 
	
  $_fast_track = get_user_meta(get_current_user_id(), '_fast_track',true);
	if(!is_array($_fast_track)) $_fast_track = array();
  
  if ($_GET['fast-track']=='true') :
  	if(!in_array($post->ID,$_fast_track)) : $_fast_track[] = $post->ID; endif;
  else :
  	$_fast_track = array_diff($_fast_track, array($post->ID));	
  endif;  
  
  update_user_meta(get_current_user_id(),'_fast_track',$_fast_track);
  
endif;  

if(isset($_GET['reference-checked']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 
	
  $_reference_checked = get_user_meta(get_current_user_id(), '_reference_checked',true);
	if(!is_array($_reference_checked)) $_reference_checked = array();
  
  if ($_GET['reference-checked']=='true') :
  	if(!in_array($post->ID,$_reference_checked)) : $_reference_checked[] = $post->ID; endif;
  else :
  	$_reference_checked = array_diff($_reference_checked, array($post->ID));	
  endif;  
  
  update_user_meta(get_current_user_id(),'_reference_checked',$_reference_checked);
  
endif;  

if(isset($_GET['video-interview-evaluated']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 
	
  $_video_interview_evaluated = get_user_meta(get_current_user_id(), '_video_interview_evaluated',true);
	if(!is_array($_video_interview_evaluated)) $_video_interview_evaluated = array();
  
  if ($_GET['video-interview-evaluated']=='true') :
  	if(!in_array($post->ID,$_video_interview_evaluated)) : $_video_interview_evaluated[] = $post->ID; endif;
  
  elseif ($_GET['video-interview-evaluated']=='submitted') :
  	if(!in_array('submitted',$_video_interview_evaluated)) : $_video_interview_evaluated[] = 'submitted'; endif;
  
  else :
  	//$_video_interview_evaluated = array_diff($_video_interview_evaluated, array($post->ID));	
  		$_video_interview_evaluated = [];
  endif;  
  
  update_user_meta(get_current_user_id(),'_video_interview_evaluated',$_video_interview_evaluated);
  
endif;  

if(isset($_GET['no-red-flags']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 
	
  $_no_red_flags = get_user_meta(get_current_user_id(), '_no_red_flags',true);
	if(!is_array($_no_red_flags)) $_no_red_flags = array();
  
  if ($_GET['no-red-flags']=='true') :
  	
  if(!in_array($post->ID,$_no_red_flags)) : $_no_red_flags[] = $post->ID; endif;
  
  elseif ($_GET['no-red-flags']=='redflag') :
  
    if(!in_array('redflag',$_no_red_flags)) : $_no_red_flags[] = 'redflag'; endif;	
  
  else :
  $_no_red_flags = [];
  //$_no_red_flags = array_diff($_no_red_flags, array($post->ID));	
  endif;  
  
  update_user_meta(get_current_user_id(),'_no_red_flags',$_no_red_flags);
  
endif;  

if(isset($_GET['completed-evaluation']) && is_single() && is_user_logged_in() && current_user_can('can_submit_job') && get_post_type() == 'resume') : 
	
  $_completed_evaluation = get_user_meta(get_current_user_id(), '_completed_evaluation',true);
	if(!is_array($_completed_evaluation)) $_completed_evaluation = array();
  
  if ($_GET['completed-evaluation']=='true') :
  	if(!in_array($post->ID,$_completed_evaluation)) : $_completed_evaluation[] = $post->ID; endif;
  else :
  	$_completed_evaluation = array_diff($_completed_evaluation, array($post->ID));	
  endif;  
  
  update_user_meta(get_current_user_id(),'_completed_evaluation',$_completed_evaluation);
  
endif;  
  
}

add_action('appthemes_before_post', 'jr_processing_status');
