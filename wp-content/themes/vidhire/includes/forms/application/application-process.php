...
function jr_process_application_form() {
	...
	if (isset($_POST['apply_to_job'])) :
		...
		if ( is_uploaded_file( $_FILES['your_cv']['tmp_name'] ) ) {
			...
			if ( !in_array( $extension, $allowed ) ) $errors->add( 'file_submit_error', __('<strong>ERROR</strong>: Only pdf, zip, doc, txt and rtf files are allowed.', APP_TD ) );
		}
		...
		if ( $errors && sizeof( $errors ) > 0 && $errors->get_error_code() ) {
			...
			if ( $errors && sizeof($errors)>0 && $errors->get_error_code() ) {
				...
				// CC Admin
				if ( 'yes' == get_option('jr_bcc_apply_emails') ) :
