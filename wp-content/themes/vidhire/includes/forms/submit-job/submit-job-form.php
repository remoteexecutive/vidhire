...
//var_dump($job);
//var_dump(jr_handle_update_job_listing());
?>
...
<?php the_listing_logo_editor($job->ID); ?>
    ...
    <fieldset>
        ...
        -->
        <!--
        <p><label for="post_title"><?php _e('Job title', APP_TD); ?> <span title="required">*</span></label> <input type="text" class="text required" name="post_title" id="post_title" value="<?php echo esc_attr($job->post_title); ?>" />
        </p>
        -->      

        <p>
            <label for="job_term_cat">Job Title<span title="required">*</span></label>
            <input type="text" name="job_term_cat" id="job_term_cat" class="job_term_cat text valid" value="<?php echo esc_attr($job->job_term_cat); ?>"> 
        </p>
...
if ($job_types && sizeof($job_types) > 0) {
    foreach ($job_types as $type) {
        ...
        <!--
        <p class="optional">
        <label for="resume_cat">Job Category</label>
                          <input type="text" name="resume_cat" id="resume_cat" class="resume_cat text valid" value="<?php esc_attr($job->category); ?>" />
        </p>
        -->
        ...
        <!--
        <p>
         <label for="job_term_cat">Job Category<span title="required">*</span></label>
         <input type="text" name="job_term_cat" id="job_term_cat" class="job_term_cat text valid" value="<?php echo esc_attr($job->job_term_cat); ?>"> 
        </p>
        -->

        <!--
                          <p class="<?php if (!$cat_required) : echo 'optional';
        endif; ?>"><label for="job_cat"><?php _e('Job Category', APP_TD); ?> <?php if ($cat_required) : ?><span title="required">*</span><?php endif; ?></label> <?php
        $args = array(
        ...
        <p class="optional">
        <label for="resume_cat">Job Unique ID</label>
                          <input type="text" name="resume_cat" id="resume_cat" class="resume_cat text valid" value="" />
        </p>
        -->
        <p class="optional"><label for="tax_input[<?php echo APP_TAX_TAG; ?>]"><?php _e('Job Category (comma separated)', APP_TD); ?></label> <input type="text" class="text" name="tax_input[<?php echo APP_TAX_TAG; ?>]" value="<?php the_job_listing_tags_to_edit($job->ID); ?>" id="tax_input[<?php echo APP_TAX_TAG; ?>]" /></p>
    </fieldset>
    ...
    <fieldset>
        <legend><?php _e('Video Link')?></legend>
        <p><?php _e('Copy the video link from the browser', APP_TD);?><p>
        <input type="text" name="job_listing_video" class="text" value="<?php echo esc_attr($job->job_listing_video); ?>"/>
    </fieldset>
    ...
    <p><input type="submit" class="submit" name="job_submit" value="Submit Job" /></p>
    <!--    
    <p><input type="submit" class="submit" name="job_submit" value="<?php echo esc_attr($submit_text); ?>" /></p>
    <p><input type="submit" class="submit" name="job_confirm" value="<?php echo esc_attr($submit_text); ?>" /></p>    
    <p><input type="submit" class="submit" name="preview_submit" value="<?php echo esc_attr($submit_text); ?>" /></p>
    -->
...
?>
        ...
        $("input#job_city").autocomplete({
            ...
            search: function () {
                ...
                if (c_val == 'United States' || c_val.val() == 'USA' || c_val == 'US')
                    return true;
                else
                    return false;
            }
