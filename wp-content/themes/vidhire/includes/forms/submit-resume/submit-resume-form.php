...
function jr_submit_resume_form( $resume_id = 0 ) {
	...
	?>" method="post" enctype="multipart/form-data" id="submit_form" class="submit_form main_form">
		...
		<fieldset>
			...
			<p><label for="summary"><?php _e('Objective', APP_TD); ?> <span title="required">*</span></label> <textarea rows="5" cols="30" name="summary" id="summary" placeholder="<?php _e('Describe your career objectives.', APP_TD); ?>" class="short" style="height:100px;"><?php if (isset($posted['summary'])) echo $posted['summary']; ?></textarea></p>
    ...
    <fieldset>
      	...
      	<?php if (isset($posted['overall_average']) && 
        ...
        <?} else {?>
      	...
      	<?php if (isset($posted['overall_average']) && 
                  $posted['overall_average'] == '70% - 75%') { ?>
      	
      		<input checked="checked" type="radio" name="overall_average" value="70% - 75%" />
      	
      	<?} else {?>
      		<input type="radio" name="overall_average" value="70% - 75%" />
      	<?php } ?>
      	<label class="overall_average">70% - 75%</label>
      	
      	<?php if (isset($posted['overall_average']) && 
                  $posted['overall_average'] == '75% - 80%') { ?>
      	
      		<input checked="checked" type="radio" name="overall_average" value="75% - 80%" />
      	<?php } else { ?>
      		<input type="radio" name="overall_average" value="75% - 80%" />
      	<?php }?>
      	<label class="overall_average">75% - 80%</label>
      	
      	<?php if (isset($posted['overall_average']) && 
                  $posted['overall_average'] == '80% - 85%') { ?>
      	
      	<input checked="checked" type="radio" name="overall_average" value="80% - 85%" />
      	
      	<?php } else { ?>
      		<input type="radio" name="overall_average" value="80% - 85%" />
      	<?php }?>
      	<label class="overall_average">80% - 85%</label>
      	
      	<?php if (isset($posted['overall_average']) && 
                  $posted['overall_average'] == '85% - 90%') { ?>
      	
      	<input checked="checked" type="radio" name="overall_average" value="85% - 90%" />
      	<?php } else {?>
      	<input type="radio" name="overall_average" value="85% - 90%" />
      	<?php } ?>
      	<label class="overall_average">85% - 90%</label>
      ...
      <p class="optional">
     	<label for="degree_date_issued"><?php _e('Date Issued', APP_TD); ?></label>
     	<input class="text" type="date" name="degree_date_issued" value="<?php if (isset($posted['degree_date_issued'])) echo $posted['degree_date_issued']; ?>"
       id="degree_date_issued" placeholder="<?php _e('Date Issued', APP_TD); ?>" />   
     </p>
    ...
    <fieldset>
    	<legend><?php _e('Carreer Map',APP_TD)?></legend>
    
   <!--Reference 1-->   
   <fieldset>
    	<legend><?php _e('Company 1',APP_TD)?></legend>
...
"Rocky marriage");
    ...
    <fieldset>
    	<legend><?php _e('Company 2',APP_TD)?></legend>
    ...
    <!--Reference 3-->     
    <fieldset>
    	<legend><?php _e('Company 3',APP_TD)?></legend>
