...
function jr_register_form( $redirect = '', $role = 'job_seeker' ) {
				...
				<?php 
					if ( 'yes' == get_option('jr_allow_job_seekers') ) :
						if ( ! $role || 'yes' == get_option( $app_abbr.'_allow_recruiters' ) ) :
							?>        		
							<p class="role login_account_type">
								...
								<?php if ( ! is_page( JR_Job_Submit_Page::get_id() ) ): ?>
									<label class="alt"><input type="radio" tabindex="6" name="role" value="job_seeker" <?php checked( ( ( isset($posted['role']) && $posted['role']=='job_seeker' ) || ! isset($posted['role']) ) ); ?> /> <?php _e( 'I am a <strong>Job Seeker</strong>', APP_TD ); ?></label>
								<?php endif; ?>
							</p>
