...
function jr_handle_job_submit_form() {
    ...
    if ('edit-job' == $_POST['action']) {
        ...
        //update sidebar data for Resume Statuses
        global $wpdb;

        $job_title = $_POST['job_term_cat'];

        $sql = $wpdb->prepare("UPDATE wp_resume_statuses 
			       SET job_applied_to = %s, job_slug = %s
                               WHERE job_id = %d", $job_title, sanitize_title($job_title), $job->ID);
        $wpdb->query($sql);
    ...
    //redirect to Completed Job Page immediately
    wp_redirect(get_permalink($job->ID));
    exit();
}
...
function jr_handle_update_job_listing() {
    ...
    //Get Job Category
    $job_cat_name = $_POST['job_term_cat'];
    $job_id = intval($_POST['ID']);

    //if (term_exists($get_job_applied_to->job_applied_to, APP_TAX_RESUME_CATEGORY)) {
        /* $cat_defaults = array(
          'cat_name' => $job_cat,
          'taxonomy' => APP_TAX_RESUME_CATEGORY); */

    
    $job_applied_to = $wpdb->get_row("SELECT job_applied_to as job_name FROM wp_resume_statuses WHERE job_id = $job_id");
    
    //This will insert a new job category or update it if it already exists
    if($job_cat_name != $job_applied_to->job_name) {
        
        $sql_terms = $wpdb->prepare("UPDATE wp_terms 
			       SET name = %s, slug = %s
                               WHERE name = %s",$job_cat_name,sanitize_title($job_cat_name),$job_applied_to->job_name);
        $wpdb->query($sql_terms);
        
        $sql_terms_taxonomy = $wpdb->prepare("UPDATE wp_term_taxonomy 
			       SET description = %s
                               WHERE description = %s AND taxonomy in ('')",$job_cat_name,$job_applied_to->job_name,APP_TAX_RESUME_CATEGORY);
        $wpdb->query($sql_terms_taxonomy);
        
    } else {

        wp_insert_term(
                $job_cat_name, // the term
                APP_TAX_RESUME_CATEGORY, // the taxonomy
                array(
            'description' => $job_cat_name,
            'slug' => strtolower($job_cat_name)
                )
        );
    }
    ...
    $job_cat = jr_get_listing_tax('job_term_cat', APP_TAX_RESUME_CATEGORY);
    $job_type = jr_get_listing_tax('job_term_type', APP_TAX_TYPE);
    $job_salary = jr_get_listing_tax('job_term_salary', APP_TAX_SALARY);

    $args = wp_array_slice_assoc($_POST, array('ID', 'post_title', 'post_name', 'post_content', 'tax_input', 'job_term_cat', 'job_listing_video'));

    $args['post_title'] = $job_cat_name; //job category will be job category
    $args['post_name'] = sanitize_title($job_cat_name);
    $args['post_content'] = jr_maybe_strip_tags($args['post_content'], 'post_content');
    $args['post_type'] = APP_POST_TYPE;
    $args['job_term_cat'] = $_POST['job_term_cat'];
    $args['job_listing_video'] = $_POST['job_listing_video'];
    ...
    if (empty($job)) {
        ...
        $wpdb->insert(
                'wp_spreadsheet', array(
            'user_id' => wp_get_current_user()->ID,
            'job_id' => intval($_POST['ID']),
            'spreadsheet_data' => $_POST['tableData']
                ), array(
            '%d',
            '%d',
            '%s'
    ...
    wp_set_object_terms($job_id, (int) $job_cat, APP_TAX_RESUME_CATEGORY);
...
function jr_get_job_listing_fields() {
    $fields = array(
        ...
        'website' => '_CompanyURL',
        'apply' => '_how_to_apply',
        'job_term_cat' => '_job_term_cat',
        'job_listing_video' => 'job_listing_video',
    );
...
function jr_job_required_fields() {
    ...
    $required = array(
        ...
        'job_term_cat' => __('Job Category', APP_TD)
            ,
    );
...
function jr_get_default_job_to_edit() {
    ...
    if ($job_id = get_query_var('job_id')) {
        ...
        $job_cat_tax = jr_get_the_job_tax($job->ID, APP_TAX_RESUME_CATEGORY);
        ...
        $job->category = jr_get_listing_tax('job_term_cat', APP_TAX_RESUME_CATEGORY);
...
function jr_get_listing_cat_id() {
    ...
    if (is_null($cat_id)) {
        if (isset($_REQUEST['_' . APP_TAX_RESUME_CATEGORY]) && $_REQUEST['_' . APP_TAX_RESUME_CATEGORY] != -1) {
            $listing_cat = get_term($_REQUEST['_' . APP_TAX_RESUME_CATEGORY], APP_TAX_RESUME_CATEGORY);
            $cat_id = is_wp_error($listing_cat) ? false : $listing_cat->term_id;
        } else {
            $cat_id = false;
        }
...
function jr_job_details($job_id = 0) {
    ...
    $cat_terms = get_the_job_terms($job_details->ID, APP_TAX_RESUME_CATEGORY);

    if ($cat_terms)
    //$category = get_the_job_terms( $job_details->ID, APP_TAX_CAT )->term_id;
        $category = get_the_job_terms($job_details->ID, APP_TAX_RESUME_CATEGORY)->term_id;
